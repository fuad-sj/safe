...
class _OTPTextFieldState extends State<OTPTextField> {
  ...
  static const int FOCUS_PUSHED_NONE = -1;
  static const int FOCUS_PUSHED_LEFT = 0;
  static const int FOCUS_PUSHED_RIGHT = 1;

  int _previousFocusIndex = -1;
  int _previousFocusPush = FOCUS_PUSHED_NONE;
    ...
    return Container(
      ...
      child: TextField(
        ...
        onChanged: (String str) {
          int focus_pushed_direction = FOCUS_PUSHED_NONE;
          void _updatePreviousFocusState() {
            _previousFocusPush = focus_pushed_direction;
            if (focus_pushed_direction == FOCUS_PUSHED_NONE) {
              _previousFocusIndex = -1;
            } else {
              _previousFocusIndex = index;
            }
          }

          if (str.isEmpty) {
            if (index != 0) {
              focus_pushed_direction = FOCUS_PUSHED_LEFT;
              _updatePreviousFocusState();
              _focusNodes[index]!.unfocus();
              _focusNodes[index - 1]!.requestFocus();
            }
          } else {
            // if overwriting an existing cell, replace value
            if (str.length > 1) {
              if ((index + 1 != widget.length) &&
                  _previousFocusPush != FOCUS_PUSHED_NONE) {
                _textControllers[index]?.text = str[0];
                _textControllers[index + 1]?.text = str[1];

                _pin[index] = str[0];
                _pin[index + 1] = str[1];

                focus_pushed_direction = FOCUS_PUSHED_RIGHT;

                // just for the purpose of setting _pin in the setState 10 lines below. TODO: find better way to update that
                str = str[0];
              } else {
                str = str[1];
                _textControllers[index]?.text =
                    str; // actually overwrite cell value

                // overwriting the text, doesn't push focus, it stays where it is
                focus_pushed_direction = FOCUS_PUSHED_NONE;
              }
            } else {
              // by default, writing text pushes the focus to the right
              focus_pushed_direction = FOCUS_PUSHED_RIGHT;
            }
            _updatePreviousFocusState(); // before changing focuses, update previous state
  ...
  void handleFocusChange(int index) {
    ...
    /**
     * If the current focused cell didn't "gain" its focus by pushes from right or left, reset _previous
     */
    bool reset_focus_push = false;
    if (_previousFocusIndex == -1) {
      reset_focus_push = true;
    } else {
      if (_previousFocusIndex == (index - 1)) {
        if (_previousFocusPush != FOCUS_PUSHED_RIGHT) {
          reset_focus_push = true;
        }
      } else if (_previousFocusIndex == (index + 1)) {
        if (_previousFocusPush != FOCUS_PUSHED_LEFT) {
          reset_focus_push = true;
        }
      } else {
        reset_focus_push = true;
    ...
    // if the index has text, make teh cursor be after the character to allow deletion
